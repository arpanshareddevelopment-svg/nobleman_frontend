    ...
    <div ref={ref} onMouseEnter={show} onMouseLeave={hide} className="relative">
      ...
      {pos && (
        <div
          ...
          style={{ top: pos.top, left: 74, transform: "translateY(-50%)" }}
        >
          {label}
        </div>
      )}
...
export default function Sidebar({ activePage, setPage, openAddAdmin, openAddStudent }: Props) {
  ...
  // ─── Nav Item ──────────────────────────────────────────────────────────────
  const navItem = (id: PageId, icon: React.ReactNode, label: string, badge?: number) => {
    ...
    return (
      <NavTooltip label={label} collapsed={collapsed}>
        <div
          ...
          style={{ color: isActive ? "#111827" : "var(--sb-text)" }}
        >
          <span className="flex w-5 shrink-0 items-center justify-center">{icon}</span>
          {!collapsed && <span className="flex-1 truncate">{label}</span>}
          {!collapsed && badge ? (
            <span className="ml-auto rounded-full bg-emerald-500 px-1.5 py-0.5 text-[10px] font-bold text-white">{badge}</span>
          ) : null}
          {collapsed && badge ? (
  ...
  const subItem = (label: string, onClick: () => void, isActive = false) => (
    <div
      ...
      className={clsx(
        ...
        isActive ? "text-emerald-500" : "text-[var(--sb-sub-text)] hover:bg-[var(--sb-hover)]",
      )}
    >
  ...
  const collapsibleSection = (
    ...
    return (
      <div className="relative">
          ...
          >
            ...
            {!collapsed && (
              <>
                ...
                <span className={clsx("transition-transform duration-200", isOpen && "rotate-90")}>
        ...
        {!collapsed && (
          <div className={clsx("overflow-hidden transition-all duration-200", isOpen ? "max-h-80" : "max-h-0")}>
            {children}
          </div>
        )}
  ...
  const sectionLabel = (text: string) =>
    collapsed ? (
      <div
        ...
        style={{ background: "linear-gradient(to right, transparent, var(--sb-divider), transparent)" }}
      />
    ) : (
      <div
    ...
    >
        ...
        {!collapsed ? (
          <>
            ...
            <button
              onClick={() => {
                setCollapsed(true);
                setOpenSubs({ "sub-courses": false, "sub-students": false, "sub-instructors": false, "sub-exams": false });
              }}
          ...
          <div className="relative flex w-full items-center justify-center">
            <div
              ...
              style={{ borderColor: "var(--sb-logo-border)" }}
            >
              <Image
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden px-1.5 py-2">
        ...
        <div className="py-1">
          ...
          {collapsibleSection("sub-courses", <BookOpen size={17} />, "Courses", "instructors", <>
            {subItem("All Courses", () => setPage("instructors"), activePage === "instructors")}
            {subItem("Add Course", openAddAdmin)}
            {subItem("Course Content", () => setPage("instructors"))}
          </>)}
          {collapsibleSection("sub-students", <Users size={17} />, "Students", "Students", <>
            {subItem("All Students", () => setPage("Students"), activePage === "Students")}
            {subItem("Add Student", openAddStudent)}
            {subItem("Enrollments", () => setPage("Students"))}
          </>)}
          {collapsibleSection("sub-instructors", <UserCheck size={17} />, "Instructors", "instructors", <>
            {subItem("All Instructors", () => setPage("instructors"), activePage === "instructors")}
            {subItem("Add Instructor", () => setPage("instructors"))}
          </>)}
          {collapsibleSection("sub-exams", <FileText size={17} />, "Quiz / Exams", "exams", <>
        ...
        <div className="py-1">
          ...
          {navItem("tickets", <MessageSquare size={17} />, "Support Tickets", 5)}
      ...
      <div
        ...
        style={{ borderColor: "var(--sb-border)", background: "var(--sb-footer-bg)" }}
      >
        {!collapsed ? (
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold" style={{ color: "var(--sb-footer-name)" }}>
                Super Admin
              </div>
              <div className="truncate text-[11px]" style={{ color: "var(--sb-footer-email)" }}>
                admin@platform.com
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-amber-400 text-sm font-black text-gray-900">
              SA
            </div>
          </div>
        )}
